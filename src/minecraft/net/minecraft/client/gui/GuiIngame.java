...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(float partialTicks) {
        ...
        Render.Render2D renderEvent = new Render.Render2D(partialTicks, scaledresolution);
        renderEvent.call();
       ...
       // this.renderPotionEffects(scaledresolution);
        ...
        if (scoreobjective1 != null) {
            if (!(store.yaff.feature.impl.render.Scoreboard.noScoreboard.getBooleanValue() && Objects.requireNonNull(Yaff.of.featureManager.getFeature(store.yaff.feature.impl.render.Scoreboard.class)).getState())) {
...
//    }
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes) {
        ...
        for (Score ignored : collection) {
            ++j;
            k2 = j1 - j * this.getFontRenderer().FONT_HEIGHT;
            l2 = scaledRes.getScaledWidth() - 3 + 2;
        }
        Rounded.drawRound(l1 - 3, k2 - this.getFontRenderer().FONT_HEIGHT - 2, scaledRes.getScaledWidth() - 3 + 2 - l1, 12 + (collection.size()) * this.getFontRenderer().FONT_HEIGHT + 3, store.yaff.feature.impl.render.Scoreboard.roundRadius.getNumericValue(), 1342177280);
        //this.getFontRenderer().drawString(ChatFormatting.YELLOW + "www.yaff.store", l1, k2 + collection.size() * this.getFontRenderer().FONT_HEIGHT + 2, 553648127);
        j = 0;
        for (Score score1 : collection) {
            ...
            if (!(store.yaff.feature.impl.render.Scoreboard.removeNumbers.getBooleanValue() && Objects.requireNonNull(Yaff.of.featureManager.getFeature(store.yaff.feature.impl.render.Scoreboard.class)).getState())) {
                this.getFontRenderer().drawString(s2, l - 5 - this.getFontRenderer().getStringWidth(s2), k, 553648127);
            }
