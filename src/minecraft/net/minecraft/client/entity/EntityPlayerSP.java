...
import store.yaff.feature.impl.player.Freecam;
import store.yaff.feature.impl.world.PortalChat;
...
public class EntityPlayerSP extends AbstractClientPlayer {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        DamageEvent damageEvent = new DamageEvent(amount);
        damageEvent.call();
        return false;
    }
    ...
    public void heal(float healAmount) {
        HealEvent healEvent = new HealEvent(healAmount);
        healEvent.call();
    }
    ...
    private void onUpdateWalkingPlayer() {
        MotionEvent motionEvent = new MotionEvent(this.mc.player.rotationYaw, this.mc.player.rotationPitch, this.mc.player.posX, this.mc.player.posY, this.mc.player.posZ, this.mc.player.motionX, this.mc.player.motionY, this.mc.player.motionZ, getEntityBoundingBox().minY, this.mc.player.onGround, this.isSprinting(), this.isSneaking());
        motionEvent.call();
        if (motionEvent.isCancelled()) {
            return;
        }
        boolean flag = motionEvent.isSprinting();
        ...
        boolean flag1 = motionEvent.isSneaking();
        ...
        if (this.isCurrentViewEntity()) {
            ...
            double d1 = motionEvent.getMinY() - this.lastReportedPosY;
            ...
            if (this.isRiding()) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(motionEvent.getMotionX(), -999.0D, motionEvent.getMotionZ(), motionEvent.getYaw(), motionEvent.getPitch(), motionEvent.isOnGround()));
                flag2 = false;
            } else if (flag2 && flag3) {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(motionEvent.getX(), motionEvent.getMinY(), motionEvent.getZ(), motionEvent.getYaw(), motionEvent.getPitch(), motionEvent.isOnGround()));
            } else if (flag2) {
                if (!Objects.requireNonNull(Yaff.of.featureManager.getFeature(Freecam.class)).getState()) {
                    this.connection.sendPacket(new CPacketPlayer.Position(motionEvent.getX(), motionEvent.getMinY(), motionEvent.getZ(), motionEvent.isOnGround()));
                }
            } else if (flag3) {
                this.connection.sendPacket(new CPacketPlayer.Rotation(motionEvent.getYaw(), motionEvent.getPitch(), motionEvent.isOnGround()));
            } else if (this.prevOnGround != motionEvent.isOnGround()) {
                this.connection.sendPacket(new CPacketPlayer(motionEvent.isOnGround()));
            }
            if (flag2) {
                this.lastReportedPosX = motionEvent.getX();
                this.lastReportedPosY = motionEvent.getMinY();
            ...
            this.prevOnGround = motionEvent.isOnGround();
    ...
    public void sendChatMessage(String message) {
        if (message.startsWith(AbstractCommand.getCommandPrefix())) {
            String[] commandArgs = net.minecraft.util.StringUtils.stripControlCodes(StringUtils.normalizeSpace(message)).substring(AbstractCommand.getCommandPrefix().length()).split(" ");
            AbstractCommand parsedCommand = Yaff.of.commandManager.getCommand(commandArgs[0]);
            if (parsedCommand != null) {
                parsedCommand.onCommand(commandArgs);
            } else {
                Chat.addChatMessage("Command not found");
            }
            return;
        }
        Message.Send messageEvent = new Message.Send(message);
        messageEvent.call();
        if (messageEvent.isCancelled()) {
            return;
        }
        this.connection.sendPacket(new CPacketChatMessage(messageEvent.getMessage()));
    }
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z) {
        Push.Block pushEvent = new Push.Block();
        pushEvent.call();
    ...
    public void onLivingUpdate() {
        ...
        if (this.inPortal) {
            if (this.mc.currentScreen != null && !this.mc.currentScreen.doesGuiPauseGame() && !Objects.requireNonNull(Yaff.of.featureManager.getFeature(PortalChat.class)).getState()) {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        MoveEvent moveEvent = new MoveEvent(p_70091_2_, p_70091_4_, p_70091_6_);
        moveEvent.call();
