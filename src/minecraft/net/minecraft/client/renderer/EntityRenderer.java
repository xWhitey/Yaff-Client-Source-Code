...
import store.yaff.feature.impl.combat.Reach;
import store.yaff.feature.impl.render.CameraClip;
import store.yaff.feature.impl.render.CustomFog;
import store.yaff.feature.impl.render.NightMode;
import store.yaff.helper.Color;
import store.yaff.ui.menu.GuiMainMenu;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null)
        {
            ...
            if (this.pointedEntity != null && flag && vec3d.distanceTo(vec3d3) > (Yaff.of.featureManager.getFeature(Reach.class).getState() ? Reach.reachExpand.getNumericValue() : 3.0D))
            {
                this.pointedEntity = null;
        ...
        else
        {
            ...
            if (flag)
            {
                ...
                if (Config.zoomMode)
                {
                    ...
                    {
                        this.scopeMath += 4 * Yaff.of.getDeltaTime();
                    }
                }
            }
            else
    ...
    {
        Entity entity = this.mc.getRenderViewEntity();
        float f = Objects.requireNonNull(entity).getEyeHeight();
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (!Objects.requireNonNull(Yaff.of.featureManager.getFeature(CameraClip.class)).getState()) {
                        RayTraceResult raytraceresult = this.mc.world.rayTraceBlocks(new Vec3d(d0 + (double) f3, d1 + (double) f4, d2 + (double) f5), new Vec3d(d0 - d4 + (double) f3 + (double) f5, d1 - d6 + (double) f4, d2 - d5 + (double) f5), false, true, false);
    ...
    private void updateLightmap(float partialTicks)
    {
        if (this.lightmapUpdateNeeded)
        {
            ...
            if (world != null)
            {
                ...
                if (Yaff.of.featureManager.getFeature(NightMode.class).getState()) {
                    int color = Color.rgb(133,133,199);
                    int r = color >> 16 & 0xFF;
                    int g = color >> 8 & 0xFF;
                    int b = color & 0xFF;
                    int a = color >> 24 & 0xFF;
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Render.Render3D renderEvent = new Render.Render3D(partialTicks);
        renderEvent.call();
    ...
    private void updateFogColor(float partialTicks)
    {
        World world = this.mc.world;
        Entity entity = this.mc.getRenderViewEntity();
        float f = Yaff.of.featureManager.getFeature(CustomFog.class).getState() ? CustomFog.distance.getNumericValue() * 10 : 0.25F + 0.75F * (float)this.mc.gameSettings.renderDistanceChunks / 32.0F;
    ...
    private void setupFog(int startCoords, float partialTicks)
    {
        ...
        else
        {
            ...
            {
                float dist = Yaff.of.featureManager.getFeature(CustomFog.class).getState() ? -CustomFog.distance.getNumericValue() : Config.getFogStart();
        ...
        FogEvent event = new FogEvent(fogColorRed, fogColorGreen, fogColorBlue, 0);
        event.call();

        fogColorRed = event.getR() / 255F;
        fogColorGreen = event.getG() / 255F;
        fogColorBlue = event.getB() / 255F;
