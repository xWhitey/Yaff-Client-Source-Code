...
import store.yaff.feature.impl.render.Animations;
import store.yaff.feature.impl.render.ViewModel;
...
public class ItemRenderer {
    ...
    private void transformEatFirstPerson(float p_187454_1_, EnumHandSide p_187454_2_, ItemStack p_187454_3_) {
        ...
        if (Yaff.of.featureManager.getFeature(ViewModel.class).getState()) {
    ...
    private void transformSideFirstPerson(EnumHandSide p_187459_1_, float p_187459_2_) {
        TransformSideFirstPerson event = new TransformSideFirstPerson(p_187459_1_);
        event.call();
        if (Yaff.of.featureManager.getFeature(ViewModel.class).getState()) {
    ...
    public void translate() {
        ...
        GlStateManager.rotate(20.0f, 0.0f, 1.0f, 0.0f);
        GlStateManager.scale(Animations.itemSize.getNumericValue(), Animations.itemSize.getNumericValue(), Animations.itemSize.getNumericValue());
    }
        ...
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(p_187457_4_)) {
            ...
            if (p_187457_6_.func_190926_b()) {
                ...
                if (p_187457_1_.isHandActive() && p_187457_1_.getItemInUseCount() > 0 && p_187457_1_.getActiveHand() == p_187457_4_) {
                    ...
                    float rotateAngles = MathHelper.sin((float) (f1 * f1 * Math.PI));
                    if (Objects.requireNonNull(Yaff.of.featureManager.getFeature(Animations.class)).getState() && enumhandside != EnumHandSide.LEFT) {
                        switch (Animations.animationMode.getListValue().toLowerCase()) {
                            case "fap" -> {
                                float sin1;
                                float sin2;
                                ...
                                GlStateManager.rotate(this.mc.player.isSwingInProgress ? (float)(-alpha) / 10 : 1.0F, 1.0F, -0.0F, 1.0F);
                                GlStateManager.translate(0.0D, 0.0D, 0.5D);
                            }
                            case "jello" -> {
                            ...
                            case "hit" -> {
                                this.transformFirstPersonItem(f / 2, swingProgress <= 0.5f ? swingProgress / 5 : swingProgress);
                                this.translate();
                                GlStateManager.rotate(rotateAngles * 30.0F / 2.0F, -rotateAngles, -0.0F, 9.0F);
                                GlStateManager.rotate(rotateAngles * 40.0F, 1.0F, -rotateAngles / 2.0F, -0.0F);
                            }
                            case "neutral" -> {
                            ...
                            case "float" -> {
                                ...
                                this.translate();
                                GlStateManager.rotate(45.0F, 0.0F, 1.0F, 0.0F);
                                float b = MathHelper.sin(swingProgress * swingProgress * (float) Math.PI) * 2;
                                float b1 = MathHelper.sin(MathHelper.sqrt(swingProgress) * (float) Math.PI) * 2;
                            ...
                            case "spin" -> {
                                this.transformFirstPersonItem(0, 0);
                                this.translate();
                                GlStateManager.rotate(spin * Animations.spinSpeed.getNumericValue(), spin, 0, spin);
                                spin++;
                            }
                            case "astolfo" -> {
                                this.transformFirstPersonItem(0, 0);
                                this.translate();
                                GlStateManager.rotate((float) (System.currentTimeMillis() / 16 * (int) Animations.spinSpeed.getNumericValue() % 360), 0.0f, 0.0f, -0.1f);
                            }
                            default -> {
