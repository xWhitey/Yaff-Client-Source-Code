...
import store.yaff.event.impl.MissEvent;
import store.yaff.event.impl.Push;
import store.yaff.feature.impl.movement.Flight;
...
public abstract class EntityPlayer extends EntityLivingBase {
            ...
            if (!targetEntity.hitByEntity(this)) {
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        ...
                        if (flag2) {
                            HitEvent hitEvent = new HitEvent(targetEntity, HitEvent.HitType.CRITICAL);
                            hitEvent.call();
                        ...
                        if (!flag2 && !flag3) {
                            if (flag) {
                                HitEvent hitEvent = new HitEvent(targetEntity, HitEvent.HitType.STRONG);
                                hitEvent.call();
                                if (hitEvent.isCancelled()) {
                                    return;
                                }
                                ...
                                HitEvent hitEvent = new HitEvent(targetEntity, HitEvent.HitType.WEAK);
                                hitEvent.call();
                                if (hitEvent.isCancelled()) {
                                    return;
                                }
                        ...
                        if (targetEntity instanceof EntityLivingBase targetEntityLivingBase) {
                            float f5 = f4 - targetEntityLivingBase.getHealth();
                            if (f5 <= 0) {
                                if (targetEntityLivingBase.hurtResistantTime < targetEntityLivingBase.maxHurtResistantTime / 2f) {
                                    MissEvent missEvent = new MissEvent(targetEntity);
                                    missEvent.call();
                                }
                            }
                        ...
                        HitEvent hitEvent = new HitEvent(targetEntity, HitEvent.HitType.NODAMAGE);
                        hitEvent.call();
                        if (hitEvent.isCancelled()) {
                            return;
                        }
    ...
    public boolean isPushedByWater() {
        Push.Water pushEvent = new Push.Water();
        pushEvent.call();
    ...
    public float getEyeHeight() {
        ...
        if (this.isPlayerSleeping()) {
            ...
            if (Yaff.of.featureManager.getFeature(Flight.class).getState()) {
                if (this.isElytraFlying() || this.height == 1F) {
                    f = 1.5F;
                }
            } else {
    ...
    public void applyEntityCollision(Entity entityIn) {
        Push.Entity pushEvent = new Push.Entity();
        pushEvent.call();
        if (pushEvent.isCancelled()) {
            return;
        }
