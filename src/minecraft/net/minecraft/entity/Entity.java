...
import store.yaff.event.impl.BoundingBoxEvent;
import store.yaff.event.impl.StrafeEvent;
import store.yaff.feature.impl.combat.HitBox;
import store.yaff.feature.impl.exploit.NoPitchLimit;
...
public abstract class Entity implements ICommandSender {
    ...
    public void setAngles(float yaw, float pitch) {
        ...
        if (!Objects.requireNonNull(Yaff.of.featureManager.getFeature(NoPitchLimit.class)).getState()) {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        BoundingBoxEvent boxEvent = new BoundingBoxEvent();
        boxEvent.call();
        if (this.noClip || boxEvent.isCancelled()) {
    ...
    public void func_191958_b(float p_191958_1_, float p_191958_2_, float p_191958_3_, float p_191958_4_) {
        StrafeEvent strafeEvent = new StrafeEvent(rotationYaw, rotationPitch, p_191958_1_, p_191958_3_, p_191958_4_, motionX, motionY, motionZ);
        strafeEvent.call();
        if (strafeEvent.isCancelled()) {
            return;
        }
        float strafeYaw = this instanceof EntityPlayerSP ? strafeEvent.getYaw() : this.rotationYaw;
        ...
        if (f >= 1.0E-4F) {
            ...
            float f1 = MathHelper.sin((float) (strafeYaw * Math.PI / 180.0F));
            float f2 = MathHelper.cos((float) (strafeYaw * Math.PI / 180.0F));
    ...
    /**
     * Returns the Y distance to the entity.
     */
    public float getYDistanceToEntity(Entity entityIn) {
        float f = (float) (this.posY - entityIn.posY);
        return MathHelper.sqrt(f * f);
    }

    /**
     * Returns the distance to the entity.
     */
    public float getDistanceHToEntity(Entity entityIn) {
        return (float) Math.hypot(this.posX - entityIn.posX, this.posZ - entityIn.posZ);
    }
    ...
    public float getCollisionBorderSize() {
        if (Objects.requireNonNull(Yaff.of.featureManager.getFeature(HitBox.class)).getState()) {
            return HitBox.hitboxExpand.getNumericValue();
        }
        return 0.0F;
    }
